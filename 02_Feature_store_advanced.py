...
# fs_table_name_users = "mc_demos.feature_store_tables.user_features_advanced"
fs.create_table(
    name=f"{catalog_name}.{fs_schema_name}.user_features_advanced", # unique table name (in case you re-run the notebook multiple times)
...
# fs_table_name_destinations = "mc_demos.feature_store_tables.destination_features_advanced"
...
fs.create_table(
    name=f"{catalog_name}.{fs_schema_name}.destination_features_advanced", # unique table name (in case you re-run the notebook multiple times)
...
fs.write_table(name=f"{catalog_name}.{fs_schema_name}.destination_features_advanced", df=destination_features_df, mode="overwrite")
...
model_feature_lookups = [
      FeatureLookup(
          table_name=f"{catalog_name}.{fs_schema_name}.destination_features_advanced",
          lookup_key="destination_id",
          timestamp_lookup_key="ts"
      ),
      FeatureLookup(
          table_name=f"{catalog_name}.{fs_schema_name}.user_features_advanced",
          lookup_key="user_id",
...
with mlflow.start_run(run_name="best_fs_model", experiment_id=summary_cl.experiment.experiment_id) as run:
  ...
  fs.log_model(
              ...
              conda_env = env,
              registered_model_name = f"{catalog_name}.{model_schema}.{model_name_advanced}"
          )
...
filter_string = f"name='{catalog_name}.{model_schema}.{model_name_advanced}'"
results = client.search_model_versions(filter_string)

# set alias to last version from results
client.set_registered_model_alias(f"{catalog_name}.{model_schema}.{model_name_advanced}", "Prod", results[0].version)
...
model_uri = f"models:/{catalog_name}.{model_schema}.{model_name_advanced}@Prod"
scored_df = fs.score_batch(model_uri, batch_scoring, result_type="boolean")
display(scored_df)
