...
# MAGIC To run this demo, just select the cluster `dbdemos-feature-store-max_carduner` from the dropdown menu ([open cluster configuration](https://e2-demo-field-eng.cloud.databricks.com/#setting/clusters/0914-181238-onfsf4k/configuration)). <br />
...
## set UC params for storing data and models
dbutils.widgets.text("unity_catalog", "mc_demos", "Unity Catalog Destination")
dbutils.widgets.text("fs_schema", "feature_store_tables", "FS Schema")
dbutils.widgets.text("models_schema", "models", "Models Schema")

catalog_name = dbutils.widgets.get("unity_catalog")
fs_schema_name = dbutils.widgets.get("fs_schema")
model_schema = dbutils.widgets.get("models_schema")

spark.sql(f"CREATE CATALOG IF NOT EXISTS {catalog_name}")
spark.sql(f"CREATE SCHEMA IF NOT EXISTS {catalog_name}.{fs_schema_name}")
...
fs.create_table(name=f"{catalog_name}.{fs_schema_name}.user_features",
...
fs.create_table(name=f"{catalog_name}.{fs_schema_name}.destination_features", 
...
fs.create_table(name=f"{catalog_name}.{fs_schema_name}.destination_location_features", 
                primary_keys="destination_id", 
...
fs.create_table(
    name=f"{catalog_name}.{fs_schema_name}.availability_features", 
...
feature_lookups = [ # Grab all useful features from different feature store tables
  FeatureLookup(
      table_name=f"{catalog_name}.{fs_schema_name}.user_features", 
      lookup_key="user_id",
      timestamp_lookup_key="ts",
      feature_names=["mean_price_7d"]
  ),
  FeatureLookup(
      table_name=f"{catalog_name}.{fs_schema_name}.destination_features", 
      lookup_key="destination_id",
      timestamp_lookup_key="ts"
  ),
  FeatureLookup(
      table_name=f"{catalog_name}.{fs_schema_name}.destination_location_features",  
      lookup_key="destination_id",
      feature_names=["latitude", "longitude"]
  ),
  FeatureLookup(
      table_name=f"{catalog_name}.{fs_schema_name}.availability_features", 
...
import mlflow
mlflow.set_registry_uri("databricks-uc") # set UC registry
...
with mlflow.start_run(run_name="best_fs_model", experiment_id=summary_cl.experiment.experiment_id) as run:
  ...
  fs.log_model(
              ...
              conda_env=env,
              registered_model_name = f"{catalog_name}.{model_schema}.{model_name_expert}"
  )
...
# get version just created and alias it to prod
from mlflow import MlflowClient
client = MlflowClient()

# Get all versions of the model filtered by name
filter_string = f"name='{catalog_name}.{model_schema}.{model_name_expert}'"
results = client.search_model_versions(filter_string)

# set alias to last version from results
client.set_registered_model_alias(f"{catalog_name}.{model_schema}.{model_name_expert}", "Prod", results[0].version)
...
model_uri = f"models:/{catalog_name}.{model_schema}.{model_name_expert}@Prod"
scored_df = fs.score_batch(model_uri, test_df, result_type="boolean")
display(scored_df)
...
create_realtime_table(f"{catalog_name}.{fs_schema_name}.destination_location_features")
create_realtime_table(f"{catalog_name}.{fs_schema_name}.destination_features")
create_realtime_table(f"{catalog_name}.{fs_schema_name}.user_features")
#Keep availability in streaming
create_realtime_table(f"{catalog_name}.{fs_schema_name}.availability_features", 
                      streaming = True, 
...
# MAGIC %md
# MAGIC ### Instead of going to Serving on the below UI, you can go find the model in UC and enable serving from there. 
...
latest_model = client.get_model_version_by_alias(catalog_name+"."+model_schema+'.'+model_name_expert, "Prod")
...
serving_client.create_enpoint_if_not_exists("mc_demos_uc_travel_expert", model_name=model_name_expert, model_version = latest_model.version, workload_size="Small", scale_to_zero_enabled=True, wait_start = True)
...
endpoint_url = f"{serving_client.base_url}/realtime-inference/mc_demos_uc_travel_expert/invocations"
