...
fs.create_table(
    name=f"{catalog_name}.{fs_schema_name}.travel_recommender_basic", # unique table name (in case you re-run the notebook multiple times)
    primary_keys=["id"],
    df=df.to_spark(),
...
fs_get_table = fs.get_table(f"{catalog_name}.{fs_schema_name}.travel_recommender_basic")
print(f"Feature Store Table= {catalog_name}.{fs_schema_name}.travel_recommender_basic. Description: {fs_get_table.description}")
...
model_feature_lookups = [
      FeatureLookup(
          table_name=f"{catalog_name}.{fs_schema_name}.travel_recommender_basic",
          lookup_key=["id"],
...
with mlflow.start_run(run_name="lightGBM") as run:
  ...
  fs.log_model(
              ...
              registered_model_name = f"{catalog_name}.{model_schema}.{model_registry_name}" # register your best model in UC
          )
...
# MAGIC %md 
# MAGIC
# MAGIC ### Add "production" alias to model registered in UC
# MAGIC
...
filter_string = f"name='{catalog_name}.{model_schema}.{model_registry_name}'"
results = client.search_model_versions(filter_string)

# add alias
client.set_registered_model_alias(f"{catalog_name}.{model_schema}.{model_registry_name}", "Prod", results[0].version)
...
model_uri = f"models:/{catalog_name}.{model_schema}.{model_registry_name}@Prod"
scored_df = fs.score_batch(model_uri, id_to_forecast, result_type="boolean")
display(scored_df)
